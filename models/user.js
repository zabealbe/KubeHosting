...
userSchema.pre('save', function(next) {
    if (this.isNew) {
        this.getPlan().then(plan => {
            kubernetesController.createNamespace(this._id.valueOf(), plan.limits.cpu, plan.limits.mem).then(() => {
                next();
            }).catch(err => {
                next(err);
            });
...
userSchema.pre('save', function(next) {
    if (this.isNew || this.isModified('plan.plan')) {
        this.getPlan().then(plan => {
            kubernetesController.updateResourceQuota(this._id.valueOf(), plan.limits).then(() => {
                next();
            }).catch(err => {
                next(err);
            });
